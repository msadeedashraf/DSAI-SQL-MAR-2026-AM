...
DECLARE @my_numrows INT = 5, @my_catid INT = 3;



--Use variables to pass parameters to procedure
EXEC Production.ProdsByCategory @numrows = @my_numrows, @catid = @my_catid;

EXEC Production.ProdsByCategory @numrows = 3, @catid = 2;
...
SET @var2 = N'Sadeed is teaching sql';
DECLARE @var3 AS NVARCHAR(20);
SELECT @var3 = firstname 
	FROM HR.Employees
	WHERE empid=6;
SELECT @var1 AS var1, @var2 AS var2, @var3 AS var3;
GO

select 99 , 'string', 'Mike'
