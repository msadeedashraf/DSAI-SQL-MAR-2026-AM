...
EXEC Sales.GetCustPhone @custid=@customerid, @p_phone=@phonenum OUTPUT;
SELECT @customerid AS custid, @phonenum AS phone
;



declare @my_name varchar(15) = 'Sadeed'

select @my_name , e.*  from hr.Employees as e


declare @todays_date date = getdate()
declare @my_salary int  = 5000
select @todays_date exit_date, e.*, @my_salary salary from hr.Employees as e


declare @todays_date date = getdate()
declare @my_salary int 
set @my_salary = 4000
select @todays_date exit_date, e.*, @my_salary salary from hr.Employees as e
