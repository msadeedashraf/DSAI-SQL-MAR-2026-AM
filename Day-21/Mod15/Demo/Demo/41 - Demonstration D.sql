...
DECLARE @sqlstring AS VARCHAR(1000) = 'SELECT empid, lastname FROM HR.employees;'
...
DECLARE @sqlcode AS NVARCHAR(256) =  'SELECT empid, lastname FROM HR.employees;';
EXEC sys.sp_executesql @statement = @sqlcode;
GO


EXEC sys.sp_executesql @statement = N'select * from hr.employees';
